...
def lwlr(testPoint_origin, xArr, yArr, uvArr, test_uvArr, k, down_sample_shape, lambda_limit_value=1):
    '''
    testPoint_origin: dense origin inputs
    xArr: sparse sampled inputs
    yArr: sparse sampled outputs
    uvArr: sparse sampled coordinates
    test_uvArr: dense origin coordinates
    k: params of guassian kernel function
    down_sample_shape: downsample dense values for decreasing computation.
    lambda_limit_value: L2 regulization lambda params of shift.
    '''
...
def sparse_depth_lwlr(pred_mono, guided_depth, sample_mode='grid', sample_num=100, down_sample_scale=32, lambda_limit_value=1, dataset_name=None, k_para=None):
    ...
    if sample_mode == 'uniform':
        ...
        kdtree = KDTree(uvArr)
        mask1 = kdtree.query(uvArr_step)[1]
...
if __name__ == '__main__':
    ...
    pred_mono_depth = np.load('./demo/pred_depth_mono.npy')
    gt_depth = cv2.imread('./demo/gt_depth.png', -1) / 5000.

    vmin = gt_depth[gt_depth != 0].min()
    vmax = gt_depth.max()
    os.makedirs('./demo/output_demo/', exist_ok=True)
    plt.imsave('./demo/output_demo/gt_depth_viz.png', gt_depth, cmap='rainbow', vmin=vmin, vmax=vmax)

    pred_depth_global = recover_metric_depth(pred_mono_depth, gt_depth)
    pred_depth_lwlr = sparse_depth_lwlr(pred_depth_global, gt_depth, sample_mode='grid', sample_num=100)

    plt.imsave('./demo/output_demo/pred_depth_global.png', pred_depth_global, cmap='rainbow', vmin=vmin, vmax=vmax)
    plt.imsave('./demo/output_demo/pred_depth_lwlr.png', pred_depth_lwlr, cmap='rainbow', vmin=vmin, vmax=vmax)
